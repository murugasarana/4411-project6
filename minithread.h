...
/*
 * enumeration status:
 * This enum keeps track of the possible states a given thread might be
 * in. IDLE is not a state per say, but it enables to keep track of the 
 * IDLE thead intuitively.
 */

enum status {
	OK, DESTROYED, SLEEPING, IDLE
};
...
/*
 * Return the priority of thread, 0-3
 */
extern int minithread_priority(minithread_t thread);

extern int minithread_get_status(minithread_t thread);
